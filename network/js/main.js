var sigInst, canvas, $GP, modoPan = false, manejadorSeleccionNodo
...
function atributosNodo(nodo) {
    if (!nodo) return {};
    if (nodo.attr && nodo.attr.attributes) return nodo.attr.attributes;
    if (nodo.attributes) return nodo.attributes;
    return nodo.attr || {};
}

function nombreGrupoDeNodo(nodo) {
    var grupoAttr = (config.features && config.features.groupSelectorAttribute) || "Grupo temático";
    return atributosNodo(nodo)[grupoAttr] || null;
}

function resolverClaveGrupo(clave) {
    if (!sigInst || !sigInst.clusters) return clave;
    if (sigInst.clusters[clave]) return clave;
    if (/^rgb\s*\(/i.test(clave) && sigInst.clusterColors) {
        for (var grupo in sigInst.clusterColors) {
            if (sigInst.clusterColors[grupo] === clave) return grupo;
        }
    }
    return clave;
}
...
function initSigma(config) {
    ...
    dataReady = function() {//This is called as soon as data is loaded
		a.clusters = {};
		a.clusterColors = {};
		var nodosIndex = a._core.graph.nodesIndex;
		var id, nodo, grupo;

		for (id in nodosIndex) {
			if (!nodosIndex.hasOwnProperty(id)) continue;
			nodo = nodosIndex[id];
			grupo = nombreGrupoDeNodo(nodo) || nodo.color;
			a.clusters[grupo] || (a.clusters[grupo] = []);
			a.clusters[grupo].push(id);
			a.clusterColors[grupo] = nodo.color;
		}
	
		manejadorSeleccionNodo = function (evento) {
			if (modoPan) return;
			nodeActive(evento.content[0]);
		};
		a.bind("upnodes", manejadorSeleccionNodo);

		a.draw();
		configSigmaElements(config);
		iniciarHerramientaPan();
	}
...
function configSigmaElements(config) {
    ...
    var a = [],
        b,
        colorGrupo,
        grupos = Object.keys(sigInst.clusters).sort(function (x, y) {
		...
		for (var i = 0; i < grupos.length; i++) {
			b = grupos[i];
			colorGrupo = sigInst.clusterColors[b] || "#999";
			a.push(
				'<div style="line-height:12px"><a href="#' + encodeURIComponent(b) + '" data-grupo="' + b.replace(/"/g, "&quot;") + '">' +
				'<div style="width:40px;height:12px;border:1px solid #fff;background:' + colorGrupo + ';display:inline-block"></div> ' +
				b + " (" + sigInst.clusters[b].length + ")</a></div>"
			);
		}
    //a.sort();
    $GP.cluster.content(a.join(""));
    function alturaCajaRed() {
        var el = document.querySelector(".sigma-parent");
        return el ? el.clientHeight : window.innerHeight;
    }
    function opcionesDialogoRed() {
        var alto = alturaCajaRed();
        return {
            minWidth: 400,
            maxWidth: Math.min(800, window.innerWidth - 48),
            maxHeight: Math.round(alto * 0.72),
            margin: Math.max(16, Math.round(alto * 0.04)),
            fixed: true,
            autoCenter: true,
            topRatio: 0.5,
            helpers: {
            ...
            afterShow: function () {
                var wrap = $(".fancybox-wrap.fancybox-opened");
    ...
    b = opcionesDialogoRed();
    $("a.fb").fancybox(b);
    $(window).on("resize", function () {
        if ($.fancybox.isOpen) {
            var opts = opcionesDialogoRed();
            $.extend($.fancybox.current, {
                maxHeight: opts.maxHeight,
                margin: opts.margin
            });
            $.fancybox.update();
            if ($.fancybox.current.onUpdate) {
                $.fancybox.current.onUpdate();
            }
        }
    });
...
function iniciarHerramientaPan() {
	var boton = document.getElementById("btn-pan");
	var lienzo = document.getElementById("sigma-canvas");
	if (!boton || !lienzo || !sigInst) return;

	function cursorPan() {
		if (!modoPan) return;
		var capa = sigInst._core.domElements.mouse;
		if (capa) capa.style.cursor = "grab";
		lienzo.classList.add("modo-pan");
	}

	boton.addEventListener("click", function () {
		modoPan = !modoPan;
		boton.classList.toggle("activo", modoPan);
		boton.setAttribute("aria-pressed", modoPan ? "true" : "false");

		if (modoPan) {
			cursorPan();
			if ($GP.search) $GP.search.close();
			if (sigInst && sigInst.detail) nodeNormal();
		} else {
			lienzo.classList.remove("modo-pan", "modo-pan-activo");
			var capa = sigInst._core.domElements.mouse;
			if (capa) capa.style.cursor = "default";
		}
	});

	lienzo.addEventListener("mousedown", function () {
		if (!modoPan) return;
		lienzo.classList.add("modo-pan-activo");
	});

	document.addEventListener("mouseup", function () {
		lienzo.classList.remove("modo-pan-activo");
	});
}

function actualizarBotonGrupo(nombre) {
    if (!$GP.cluster || !$GP.cluster.select) return;
    if (!nombre) {
        $GP.cluster.select.text("Seleccionar grupo").removeAttr("title");
        return;
    }
    nombre = resolverClaveGrupo(nombre);
    var color = (sigInst.clusterColors && sigInst.clusterColors[nombre]) ? sigInst.clusterColors[nombre] : "#999";
    $GP.cluster.select.html(
        '<span class="grupo-btn-muestra" style="background:' + color + '"></span>' +
        '<span class="grupo-btn-texto">' + $("<div>").text(nombre).html() + "</span>"
    );
    $GP.cluster.select.attr("title", nombre);
}
...
function Cluster(a) {
    ...
    this.content = function (a) {
        ...
        this.list.find("a").click(function (e) {
            e.preventDefault();
            var grupo = resolverClaveGrupo($(this).attr("data-grupo") || decodeURIComponent($(this).attr("href").substr(1)));
...
function nodeNormal() {
    !0 != $GP.calculating && !1 != sigInst.detail && (showGroups(!1), $GP.calculating = !0, sigInst.detail = !0, $GP.info.delay(400).animate({width:'hide'},350),$GP.cluster.hide(), sigInst.iterEdges(function (a) {
        ...
        a.attr.lineWidth = !1;
        a.attr.size = !1
    }), sigInst.draw(2, 2, 2, 2), sigInst.neighbors = {}, sigInst.active = !1, $GP.calculating = !1, window.location.hash = "", actualizarBotonGrupo())
}
...
function nodeActive(a) {
    ...
    sigInst.active = a;
    window.location.hash = encodeURIComponent(b.label);
}

function showCluster(a) {
    a = resolverClaveGrupo(a);
